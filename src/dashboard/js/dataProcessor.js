...
export class DataProcessor {
   ...
   */
  async processCSVData(csvText, filterOptions = {}) {
    ...
    try {
      ...
      // Step 2.5: Apply date filters if specified
      const filteredData = this.applyDateFilter(validatedData, filterOptions);
      this.log('info', `✅ Filtered to ${filteredData.length} records based on date range`);
      ...
      return {
        ...
        metadata: {
          ...
          originalRecords: validatedData.length,
   ...
   * @param {object} filterOptions - フィルターオプション
   * @returns {Array<object>} フィルター済みデータ
   */
  applyDateFilter(data, filterOptions = {}) {
    // フィルターが指定されていない場合は全データを返す
    if (!filterOptions || !filterOptions.period || filterOptions.period === 'all') {
      return data;
    }
    ...
    try {
      ...
      if (filterOptions.period === 'custom') {
        if (filterOptions.startDate && filterOptions.endDate) {
          ...
          this.log('warn', 'Custom period specified but dates missing, using all data');
          return data;
        }
      } 
      // プリセット期間の場合
      else {
        const dateRange = UTILS.getDateRange(filterOptions.period);
        startDate = dateRange.start;
        endDate = dateRange.end;
      }

      // 日付範囲が設定されていない場合は全データを返す
      if (!startDate || !endDate) {
        return data;
      }
      ...
      this.log('info', `📅 Applied ${filterOptions.period} filter: ${data.length} → ${filteredData.length} records`);
      
      // 最小データ点チェック
      if (filteredData.length < CONFIG.FILTERS.MIN_DATA_POINTS) {
        this.log('warn', `⚠️ Filtered data has only ${filteredData.length} records, which may not be sufficient for analysis`);
      }

      return filteredData;

    } catch (error) {
      this.log('error', `❌ Date filter error: ${error.message}`);
      // エラーが発生した場合は元のデータを返す
      return data;
    }
  }
