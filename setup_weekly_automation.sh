...
PLIST_FILE="$PROJECT_DIR/com.user.gym.analysis.weekly.plist"
...
test_run() {
    ...
    cd "$PROJECT_DIR"
    python3 weekly_automation.py --weekly
...
check_logs() {
    ...
    if [ -f "automation.log" ]; then
        echo "--- automation.log（最新50行）---"
        tail -50 automation.log
    else
        echo "❌ automation.log が見つかりません"
    fi
    
    echo ""
    
    if [ -f "automation_output.log" ]; then
        echo "--- automation_output.log（最新20行）---"
        tail -20 automation_output.log
    fi
    
    if [ -f "automation_error.log" ]; then
        echo "--- automation_error.log（最新20行）---"
        tail -20 automation_error.log
    fi
}
